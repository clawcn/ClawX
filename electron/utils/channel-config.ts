...
import { getOpenClawResolvedDir, resolveHomeDir } from './paths';
import * as logger from './logger';
import { proxyAwareFetch } from './proxy-fetch';

const OPENCLAW_DIR = join(resolveHomeDir(), '.openclaw');
...
export async function deleteChannelConfig(channelType: string): Promise<void> {
    ...
    if (channelType === 'whatsapp') {
        try {
            const whatsappDir = join(resolveHomeDir(), '.openclaw', 'credentials', 'whatsapp');
