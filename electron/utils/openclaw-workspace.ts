...
import { getResourcesDir, resolveHomeDir } from './paths';
...
async function resolveAllWorkspaceDirs(): Promise<string[]> {
  const openclawDir = join(resolveHomeDir(), '.openclaw');
  ...
  try {
    if (await fileExists(configPath)) {
      ...
      if (typeof defaultWs === 'string' && defaultWs.trim()) {
        dirs.add(defaultWs.replace(/^~/, resolveHomeDir()));
      }
      ...
      if (Array.isArray(agents)) {
        for (const agent of agents) {
          ...
          if (typeof ws === 'string' && ws.trim()) {
            dirs.add(ws.replace(/^~/, resolveHomeDir()));
          }
        }
      }
    }
